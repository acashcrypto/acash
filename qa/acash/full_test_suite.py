...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/acash/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/acash/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if acash is an ELF binary, they all are
    with open(repofile('src/acashd'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/acashd')
    ret &= test_rpath_runpath('src/acash-cli')
    ret &= test_rpath_runpath('src/acash-gtest')
    ret &= test_rpath_runpath('src/acash-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/acashd')
    ret &= test_fortify_source('src/acash-cli')
    ret &= test_fortify_source('src/acash-gtest')
    ret &= test_fortify_source('src/acash-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/acash-gtest')],
