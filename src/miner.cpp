...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
...

    {
        ...
        if (nHeight >= chainparams.GetCommunityFeeStartHeight()) {
            // Community Fee is 10% of the block subsidy
            auto vCommunityFee = txNew.vout[0].nValue * 0.1;
...
#endif
{
    ...
    try {
        while (true) {
            ...
            if (nHeight == -1) {
                LogPrintf("Error in ACASH Miner: chainActive.Height() returned -1\n");
                return;
            }
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in ACASHMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in ACASHMiner: Invalid -mineraddress\n");
                }
                return;
            }
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ACASHMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ACASHMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
