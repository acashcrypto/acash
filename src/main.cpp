...
const string strMessageMagic = "ACASH Signed Message:\n";
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In ACASH this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see ACASH protocol spec
    ...
    if (nHeight >= Params().GetCommunityFeeStartHeight()) {
        ...
        BOOST_FOREACH(const CTxOut& output, block.vtx[0].vout) {
            if (output.scriptPubKey == Params().GetCommunityFeeScriptAtHeight(nHeight)) {
                if (output.nValue == (GetBlockSubsidy(nHeight, consensusParams) * 0.1)) {
                    found = true;
                    break;
                }
            }
