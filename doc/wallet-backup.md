...
Backing up your ACASH private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
These instructions are specific for the officially supported ACASH Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`acash.conf` located in the data directory which is `~/.acash/` unless it's been overridden with `datadir=` setting):
...
Note that acashd will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`acash-cli backupwallet <nameofbackup>`.
...
If you generate a new ACASH address, it will not be reflected in the backup file.
...
`acash-cli z_exportwallet <nameofbackup>`
...
`acash-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`acash-cli z_exportkey <z-address>`
...
`acash-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`acash-cli z_importkey <z-priv-key>`
