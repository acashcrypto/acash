...
PACKAGE_VERSION=$($SRC_PATH/src/acashd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/acashd $DEB_BIN
cp $SRC_PATH/src/acash-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/acashd.1 $DEB_MAN
cp $SRC_DOC/man/acash-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/acashd.bash-completion $DEB_CMP/acashd
cp $SRC_PATH/contrib/acash-cli.bash-completion $DEB_CMP/acash-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/acashd.1
gzip --best -n $DEB_MAN/acash-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/acashd $DEB_BIN/acash-cli
